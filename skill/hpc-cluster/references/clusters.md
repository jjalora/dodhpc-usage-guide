...
|---|---|---|---|---|---|---|
| jean | jean01.arl.hpc.mil | ARL | SLURM | A100 via `aiml` gres | `--gres=aiml --gpus-per-node=N -p AIML` | 96 h |
...
to another cluster). A usage agreement must be accepted on every ssh login.
...
`WANDB_MODE=offline`. Driver 575.x (CUDA 12.9) rejects the default cu130 torch wheel and
training silently runs on CPU; `setup_cluster_env.sh` installs cu128 when the driver is < 580.
...
**makau**: typed gres is mandatory (`gpu:h100_sxm5:N` or `gpu:h100_nvl:1`); a bare
