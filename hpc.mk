# ─── HPC Cluster Makefile (hpc.mk — `include hpc.mk` from your Makefile) ─── #
...
#
# SETUP: `bash install.sh --target <repo> --dod-user <u> --anvil-user <x-u>`
# (or `make configure`) writes config.mk (gitignored) with your project name,
# usernames, and accounts. Job scripts live in scripts/slurm/ and scripts/pbs/;
# the default entry point is examples/train_smoke.py, so `make smoke` verifies
# a cluster end-to-end.

SHELL := /bin/bash
# ?= so a host Makefile that defines its own default goal keeps it.
.DEFAULT_GOAL ?= help
...
TIME      ?= 48:00:00
PARTITION ?= ai
ACCOUNT   ?= $(or $(ANVIL_ACCOUNT),nairr260061-ai)
# The AI Studio's NAIRR allocation authorizes ONLY the ai partition (gpu /
# gpu-debug reject the account). Guard it; other allocations are unaffected.
ifeq ($(strip $(ACCOUNT)),nairr260061-ai)
ifneq ($(strip $(PARTITION)),ai)
$(error anvil: allocation nairr260061-ai authorizes ONLY the ai partition; drop PARTITION=$(strip $(PARTITION)))
endif
endif
endif
...
# Default = the AI Studio project. config.mk supplies DOD_ACCOUNT / ANVIL_ACCOUNT
# so each world resolves to the right account; ACCOUNT=... on the command line wins.
ACCOUNT   ?= $(or $(DOD_ACCOUNT),OUSAF40080AIR)
...
# ─── Code sync: git (default) or rsync ─── #
# SYNC_MODE=git   — `make sync` makes the cluster checkout match GitHub (needs a
#                   per-cluster deploy key: `make deploy-key CLUSTER=<c>`).
# SYNC_MODE=rsync — `make sync` rsyncs this working tree instead. No GitHub
#                   setup needed; use it for the first smoke test or for repos
#                   that are not on GitHub. Never mix the two on one checkout.
SYNC_MODE ?= git
...
help: ## Show this help
	...
	@echo "  make deploy-key CLUSTER=raider                  # one-time per cluster: GitHub deploy key for make sync"
	@echo "  make smoke CLUSTER=raider && make smoke-wait CLUSTER=raider"
...
ifeq ($(CLUSTER),wheat)
SMOKE_IN_QUEUE = qstat -f $$ID 2>/dev/null | grep -qE "job_state = [QRHEWTS]"
else
SMOKE_IN_QUEUE = squeue -j $$ID -h -o %T 2>/dev/null | grep -q .
endif
...
smoke-wait: check-auth ## Wait for the smoke job and grade its log (exit 0 pass / 1 fail / 2 still running)
	...
	@ssh $(SSH_OPTS) $(SSH_HOST) \
		'cd $(REMOTE_DIR) && ID=$(shell cat .hpc_smoke_job) && \
		 ...
		 OUT=logs/run_$$ID.out; \
		 [ -f "$$OUT" ] || { echo "ERROR: $$OUT missing (job never started or logs/ not writable)"; ls -t logs 2>/dev/null | head; exit 1; }; \
		 echo "── $$OUT (tail) ──"; tail -25 "$$OUT"; \
		 if grep -q "SMOKE TEST PASSED" "$$OUT"; then \
		   grep -q "device=cuda" "$$OUT" || echo "WARNING: smoke ran on CPU, not CUDA — check the torch wheel vs driver (see README quirks)"; \
		   echo "RESULT: SMOKE TEST PASSED on $(CLUSTER) (job $$ID)"; \
		 else echo "── logs/run_$$ID.err (tail) ──"; tail -40 logs/run_$$ID.err 2>/dev/null; echo "RESULT: SMOKE TEST FAILED on $(CLUSTER) (job $$ID)"; exit 1; fi'
...
# ─── Cluster Operations ─── #
.PHONY: sync
ifeq ($(SYNC_MODE),rsync)
sync: check-auth ## Put this working tree on the cluster (SYNC_MODE=rsync: rsync; default git: match GitHub)
...
sync: check-auth ## Make a cluster's code checkout match GitHub (GIT_BRANCH=<branch>, default=laptop branch; SYNC_MODE=rsync to rsync instead)
	@ssh $(SSH_OPTS) $(SSH_HOST) \
...
setup-cluster: sync ## First-time cluster env setup (conda + deps)
	ssh $(SSH_OPTS) $(SSH_HOST) \
		'$(REMOTE_INIT) && cd $(REMOTE_DIR) && HPC_INSTALL_CMD="$(HPC_INSTALL_CMD)" bash scripts/setup_cluster_env.sh $(CLUSTER) $(PROJECT_NAME)'
...
# ─── Clean ─── #
# Named clean-runs (not clean) so it never shadows a host Makefile's own clean.
...
.PHONY: clean-runs
clean-runs: check-auth ## Delete run artifacts on a cluster (CLUSTER=x RUN_ID=xxx or EXCLUDE="id1 id2 ...")
